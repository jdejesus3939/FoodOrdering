...
import info.jproject.opensource.foodordering.library.DatabaseHandler;
import info.jproject.opensource.foodordering.library.HashAlgorithm;
				...
				try {
					if (json.getString(KEY_SUCCESS) != null) {
						...
						if (Integer.parseInt(res) == 1) {
							...
							// check whether password is the same as decrypted
							// ones
							if (Pcipher.equals(Penc)) {
								// user successfully logged in
								// Store user details in SQLite Database
								// DatabaseHandler db =
								// DatabaseHandler.getDatabaseHandler(getApplicationContext());
								...
								db.addUser(json_user.getString(KEY_ANUMBER),
										json_user.getString(KEY_NAME),
										json_user.getString(KEY_EMAIL),
										json_user.getString(KEY_CREATED_AT));

								// Launch Hallway Screen
								Intent hallway = new Intent(
										getApplicationContext(),
										HallwayActivity.class);
							...
							// Error in login
							loginErrorMsg
									.setText("Incorrect A number/password");
						}
					}
